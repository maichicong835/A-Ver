...
def classify_tsdr_current_status(body):
    ...
    dead_seen=any(x in text for x in dead_markers)
    active_seen=any(x in text for x in active_markers)
    if dead_seen and active_seen:
        return {"state":"UNRESOLVED","machine_bound":False,"reason":"CONTRADICTORY_TSDR_CURRENT_STATUS_MARKERS"}
    if dead_seen:
        return {"state":"DEAD_INACTIVE","machine_bound":True,"reason":"EXPLICIT_TSDR_CURRENT_STATUS_DEAD_OR_INACTIVE"}
    if active_seen:
        return {"state":"LIVE_ACTIVE","machine_bound":True,"reason":"EXPLICIT_TSDR_CURRENT_STATUS_LIVE_OR_ACTIVE"}
...
def classify_expanded_partial_recall(request,record_detail,tsdr_body,dimension):
    ...
    goods_tokens=_tokens((record_detail or {}).get("goods_services_excerpt") or "")
    ...
    return {
        "classification":"NONMATERIAL_DEAD_UNRELATED_EXPANDED_PARTIAL_RECALL" if nonmaterial else "MATERIALITY_UNRESOLVED",
