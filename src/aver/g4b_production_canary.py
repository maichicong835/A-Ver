...
def uspto_zero(browser, query):
    ...
    ok=rec.get("state")=="PASS" and (rec.get("terminal") or {}).get("native_zero") is True
    return rec,ok


def trademarkia_zero(resolver, query):
    rec=resolver.search(query,"OPAQUE_ZERO")
    ok=rec.get("state")=="CAPABILITY_PASS" and (rec.get("terminal_result") or {}).get("terminal")=="ZERO_RESULTSET"
    return rec,ok
...
def main():
    out={
        ...
        "production_shaped_canary":True,
        "fixed_phrase":True,
        "phrase":PHRASE,
        "phrase_replacement_on_failure_forbidden":True,
        "daily7_candidate":False,
        "daily7_live_gate_authorized":False,
        "may_close_g4b_only_if_full_scope_tm_pass":True,
        "phase_state":"G4B_PRODUCTION_SHAPED_PARTIAL",
        "evidence":{},
    }
    ...
    if not cores or not plan["PHONETIC_OR_SPELLING_WHEN_MATERIAL"]["queries"]:
        ...
        with sync_playwright() as p:
            ...
            tm_exact,tm_exact_ok=trademarkia_zero(tm,PHRASE)
            out["evidence"]["exact"]={"uspto":usp_exact,"trademarkia":tm_exact}

            core_rows=[]
            core_ok=True
            partial_ok=True
            tm_core_all_zero=True
            for core in cores:
                usp_core,usp_core_ok=uspto_zero(browser,official_exact_query(core))
                tm_core,tm_core_ok=trademarkia_zero(tm,core)
                usp_partial,usp_partial_ok=uspto_zero(browser,official_expanded_query(core))
                core_rows.append({
                    "core":core,
                    "uspto_exact":usp_core,
                    "trademarkia":tm_core,
                    "uspto_expanded":usp_partial,
                    "core_zero":usp_core_ok and tm_core_ok,
                    "expanded_zero":usp_partial_ok and tm_core_ok,
                })
                core_ok=core_ok and usp_core_ok and tm_core_ok
                partial_ok=partial_ok and usp_partial_ok and tm_core_ok
        ...
        related_goods_ok=exact_ok and core_ok and partial_ok and usp_phon_ok
        ...
        two_source_candidate_negative=usp_exact_ok and tm_exact_ok and core_ok and tm_core_all_zero
        decision_input={
    ...
    path=Path("artifacts/g4b-production")
    path.mkdir(parents=True,exist_ok=True)
    (path/"receipt.json").write_text(json.dumps(out,indent=2)+"\n",encoding="utf-8")
    print("phase_state=",out["phase_state"])
    print("cores=",cores)
    print("decision=",(out.get("decision") or {}).get("decision"))
    print("unresolved=",(out.get("scope") or {}).get("unresolved_dimensions"))
    if out["phase_state"]!="G4B_PRODUCTION_SHAPED_TM_PASS_PROVEN":
        raise SystemExit("G4B_PRODUCTION_SHAPED_CANARY_NOT_PASS")
