...
def main():
    out={"schema":"AVER_MATERIAL_RECORD_PRIORITY_CANARY","shadow_only":True,"candidate_breadth_expanded":False,"tm_decision_authorized":False,"legacy_record_retention_required":False,"phase_state":"MATERIAL_RECORD_PRIORITY_PARTIAL","canaries":[]}
    ...
    with sync_playwright() as p:
        ...
        for c in CANARIES:
            ...
            passed=bool(rec.get("state")=="CAPABILITY_PASS" and rec.get("post_terminal_observer_state")=="OBSERVED" and queue and pass_any and no_decisions)
            out["canaries"].append({**c,"resolver_state":rec.get("state"),"observer_state":rec.get("post_terminal_observer_state"),"anchor_count":len(anchors),"priority_queue":queue,"top4_material_candidate_seen":pass_any,"triage_does_not_decide_safety":no_decisions,"pass":passed})
        browser.close()
