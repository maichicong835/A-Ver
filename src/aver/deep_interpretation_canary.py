...
def main():
    output={"schema":"AVER_DEEP_INTERPRETATION_CANARY_RECEIPT","shadow_only":True,"candidate_breadth_expanded":False,"tm_decision_authorized":False,"daily7_live_gate_authorized":False,"canaries":[],"phase_state":"DIRECT_RECORD_DETAIL_PARTIAL"}
    ...
    with sync_playwright() as p:
        ...
        for c in CANARIES:
            ...
            passed=bool(rec.get("state")=="CAPABILITY_PASS" and (rec.get("terminal_result") or {}).get("terminal")=="POSITIVE_RESULTSET" and rec.get("post_terminal_observer_state")=="OBSERVED" and target and bound and bound.get("bound") is True and atomic_class_pass and direct_pass)
            output["canaries"].append({**c,"resolver_state":rec.get("state"),"terminal":(rec.get("terminal_result") or {}).get("terminal"),"observer_state":rec.get("post_terminal_observer_state"),"anchor_count":len(anchors),"expected_anchor_found":target is not None,"expected_anchor":target,"record_binding":bound,"atomic_class_pass":atomic_class_pass,"direct_record_binding":direct,"direct_record_pass":direct_pass,"pass":passed,"failure_signature":rec.get("failure_signature"),"observer_error":rec.get("post_terminal_observer_error")})
        browser.close()
    if len(output["canaries"])==2 and all(x["pass"] for x in output["canaries"]): output["phase_state"]="DIRECT_RECORD_DETAIL_PASS"
    ...
    for x in output["canaries"]: print(x["candidate_key"],"anchor=",x["expected_anchor_found"],"atomic=",x["atomic_class_pass"],"direct=",x["direct_record_pass"])
    if output["phase_state"]!="DIRECT_RECORD_DETAIL_PASS": raise SystemExit("DIRECT_RECORD_DETAIL_NOT_PASS")
